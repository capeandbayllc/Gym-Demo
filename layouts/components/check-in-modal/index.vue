<template>
  <div
    class="w-full h-full flex flex-col check-in-modal-content"
    ref="checkInModalContent"
  >
    <div class="page-checkin-container" ref="checkInContainer">
      <div class="check-in-page-content">
        <div
          class="wrapper p-3 w-full rounded-md"
          :class="{ 'pb-0': accountView !== null }"
        >
          ...
          <div class="bg-secondary w-full">
            <p
              class="text-center cursor-pointer align-middle mt-1"
              :class="{ 'mt-4 mb-4': isAccountViewOpen }"
              @click="
                (isAccountViewOpen = !isAccountViewOpen), (accountView = null)
              "
            >
              {{ isAccountViewOpen ? "Close" : "View Account" }}
            ...
            <div class="account-box" v-if="isAccountViewOpen">
              <ul>
                <li>
                  <button @click="changeAccountView('memberinfo')">
                    <MemberInfoIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('setting')">
                    <SettingIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('dollardoc')">
                    <DollarDocIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('announcement')">
                    <AnnouncementIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('piechart')">
                    <PieChartIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('book')">
                    <BookIcon />
                  </button>
                </li>
                <li>
                  <button @click="changeAccountView('doc')">
                    <DocIcon />
                  </button>
                </li>
              </ul>
    ...
    <div class="checkin-action-items">
      ...
      <pos-window v-if="option === 'pos'" @close="option = null" />
...
watch(option, () => {
  ...
  setTimeout(() => {
    checkInModalContent.value.scroll({
      top: scrollTO,
      left: 0,
      behavior: "smooth",
    });
