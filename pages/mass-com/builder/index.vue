<template>
  <div class="campaign-builder-container">
    <div class="page-topbar flex flex-1">Campaign Builder</div>

    <div class="page-content card-gradient-bg">
      <div class="page-content-block">
        <select-template
          v-if="activePage == 'select-template'"
          @next-page="activePage = $event"
        />
        <map-contacts
          v-if="activePage == 'map-contacts'"
          @next-page="activePage = $event"
        />
        <lead-sources
          v-if="activePage == 'lead-sources'"
          @next-page="activePage = $event"
        />
        <save-campaign
          v-if="activePage == 'save-campaign'"
          @next-page="activePage = $event"
        />
      </div>
    </div>
...
.campaign-builder-container {
  @apply pb-4 w-full h-fit pr-8 pl-5;
  .page-topbar {
