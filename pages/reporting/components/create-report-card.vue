<template>
  <card
    ...
    class="bg-neutral"
    :class="{ 'reporting-form-card': showAddNewForm }"
    :options="{
      collapse: false,
      favorite: true,
    }"
  >
    <template #content>
      <div class="card-content">
        <create-report-form
          v-if="showAddNewForm"
          @closeReportingForm="showAddNewForm = false"
        />
        <div class="filter-report-content" v-else>
          <div class="flex justify-center mt-2">
            ...
            >
              <div
                class="rounded-full w-12 h-12 m-auto text-center border-2 border-secondary hover:border-opacity-50"
              >
          ...
          <div class="flex justify-end gap-4">
            <Button
              ...
              size="sm"
              class="font-medium"
              :class="{ active: activeBtn === item.type }"
              v-for="(item, index) in actionsBtn"
              :key="index"
            >
              {{ item.name }}
            </Button>
          ...
          <div
            class="grid gap-3 mt-4 p-4 border border-secondary overflow-auto"
          >
            <div
              class="flex gap-2 justify-between"
              v-for="(item, index) in buildReportBy"
              :key="index"
            >
              <select-box
                :items="item.filter1"
              ...
              <div
                ...
                :class="{ 'equal-to-icon': calculateByDropDown === item.id }"
                ref="componentRef"
              >
                <span
                  class="equation-icon flex w-9 h-9 rounded-full text-center border-base-content bg-secondary cursor-pointer border-[3px] border-opacity-50"
                  @click="showCalculateByDropDown(item.id)"
                  >{{ item.equation }}</span
                >
                <div
                  class="calculation-dropdown"
                  v-if="calculateByDropDown === item.id"
                >
                  <div
                    class="gradient-bg border-4 border-base-content border-opacity-75 flex justify-between w-full py-3 px-2 rounded-xl"
                  >
              ...
              <img
                ...
                src="/ellipsis-icon-vertical.svg"
                alt=""
              />
              <button
...
const showCalculateByDropDown = (id) => {
  calculateByDropDown.value = !calculateByDropDown.value
    ? id
    : (calculateByDropDown.value = null);
};
</script>
