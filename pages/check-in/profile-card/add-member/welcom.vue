<template>
    <simple-card class="text-center">
        ...
        <ul class="options-wrap flex gap-8 w-[470px] overflow-auto pb-8">
            <li class="shrink-0 mx-3 cursor-pointer" :class="{'selected':option.selected}" v-for="(option, index) in options" :key="index" @click="selectOption(option)">
                <div class="h-40 w-40 p-8 border-8 border-secondary rounded-full m-6">
                    <PersonAddIcon v-if="option.title == 'Interested in Membership'" class="h-full w-full"/>
                    <PersonIcon v-else class="h-full w-full"/>
                </div>
                <span class="w-40 block">{{ option.title }}</span>
            </li>
        </ul>
    </simple-card>
</template>
