<template>
    <simple-card class="mb-8">
        <div class="grid grid-cols-3 gap-6 py-4 px-10">
            ...
            <div v-for="(item, index) in accessCode" :key="item.key" :class="index == 0 ? 'col-span-2' : 'col-span-1'" class="-lg:col-span-2 -md:col-auto mx-auto w-full">
                <div class="mb-2">{{item.label}}</div>
                <input :class="item.class" class="w-full p-1 rounded-sm" v-model="accessCodeObj[item.key]"/>
            </div>
        </div>
    </simple-card>
</template>
<script setup>
const accessCodeObj = ref({})
const accessCode = [{
    key: "access-code",
    label: "Access Code",
    class: "neutral-input"
