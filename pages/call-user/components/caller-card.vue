<template>
  <div class="call-image-container">
    <div
      ...
      :class="{
        'bg-secondary':
          userCallSetting.type === 'outgoing-call' ||
          userCallSetting.type === 'incoming-call',
        'bg-base-content/20': userCallSetting.type === 'end-call',
      }"
    >
      <img
        :src="userCallSetting.callerProfileImage"
        class="rounded-full"
        alt=""
      />
    </div>
    <div class="profile-info">
      ...
      <div
        class="call-type"
        :class="{
          'text-base-content/10': userCallSetting.type === 'outgoing-call',
          'text-secondary': userCallSetting.type === 'end-call',
          'text-success': userCallSetting.type === 'incoming-call',
        }"
      >
        {{ userCallSetting.callType }}
      </div>
      <div
        class="phone-number"
        :class="{ 'text-secondary': userCallSetting.type === 'outgoing-call' }"
      >
        {{ userCallSetting.phone }}
      </div>
      <div class="date-time" v-if="userCallSetting.type !== 'incoming-call'">
        <span :class="{ 'text-secondary': userCallSetting.type === 'end-call' }"
          >Date</span
        >
        <span :class="{ 'text-secondary': userCallSetting.type === 'end-call' }"
          >Time</span
    ...
    <div
      class="profile-call"
      :class="{
        'border-secondary bg-primary bg-opacity-40':
          userCallSetting.type === 'outgoing-call',
        'border-base-content/20 bg-base-100':
          userCallSetting.type === 'end-call',
        'border-success bg-success bg-opacity-40':
          userCallSetting.type === 'incoming-call',
      }"
    >
    ...
    <div
      ...
      v-if="userCallSetting.type !== 'incoming-call'"
    >
      <div class="mt-1">
      ...
      <div class="text-right">
        <Button class="notes h-9 min-h-0" @click="showNotes = !showNotes"
          >Notes</Button
        >
      </div>
    </div>
