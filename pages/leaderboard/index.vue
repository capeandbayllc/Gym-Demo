<template>
    <div class="page-club-container">
        ...
        <div class="-md:px-4 page-content flex-col">
            <simple-card title="Top Employee Ranking in All Categories" class="-md:text-xs">
            ...
            <simple-card title="Membership Sales">
                <div class="-md:px-4 px-12 py-3 bg-neutral">
                    <div class="flex flex-row my-4 justify-end">
                        <Button secondary size="sm">View All</Button>
                        <select-box :items="states" label="States Filter" class="w-36 mx-2"/>
                        <search-input neutral size="md" border="neutral" class="-md:w-15 w-30"/>
                    </div>
                    <div class="carousel-wrap">
                        <carousel ref="membershipCarousel" :breakpoints="breakpoints">
                            <slide v-for="(member, index) in membershipData" :key="index">
                                <simple-card class="px-4" >
                                    <span v-if="member.isFavourite" class="text-yellow-500 text-xl absolute top-0 right-2">&#9733;</span>
                                    <div class="flex items-center justify-center p-5">
                                        <img class="w-14 mr-2" src="/account.png" />
                                        <div class="text-[12px] text-left">
                                            <p>{{member.name}}</p>
                                            <p> <span class="text-secondary">Club Location:</span> {{member.clubLocation}}</p>
                                        </div>
                                    </div>
                                    <div class="p-1 border border-white rounded flex mb-4">
                                        <div class="text-center text-sm w-2/4">
                                            <p class="text-secondary my-1">Goal: <span class="ml-1">{{member.goalUnitSold}}</span></p>
                                            <ClientOnly>
                                                <apexchart
                                                    type="donut"
                                                    :height="90"
                                            ...
                                            <p class="text-secondary mb-2">Unit Sold</p>
                                            <p class="border border-sky-600 rounded w-4/5 mx-auto px-2 py-1 mb-2 text-center text-lime-500">{{member.unitSold}}</p>
                                        </div>
                                        <div class="text-center text-sm w-2/4">
                                            <p class="text-secondary my-1">Goal: <span class="ml-1">{{member.goalRevenue}}</span></p>
                                            <ClientOnly>
                                                <apexchart
                                                    type="donut"
                                                    :height="90"
                                            ...
                                            <p class="text-secondary mb-2">Dollar Amount</p>
                                            <p class="border border-sky-600 rounded w-4/5 mx-auto px-2 text-center py-1 mb-2 text-lime-500">{{member.dollarAmount}}</p>
                                        </div>
                                    </div>
                                </simple-card>
                            </slide>
                        </carousel>
                        <button class="-md:left-[0] text-secondary absolute text-3xl top-2/4 left-[-30px] translate-y-[-50%]" @click="membershipCarousel.prev()"> <next-icon/> </button>
                        <button class="-md:right-[0] text-secondary absolute text-3xl top-2/4 right-[-30px] translate-y-[-50%]" @click="membershipCarousel.next()"> <prev-icon/> </button>
                    </div>
                </div>
            </simple-card>
        </div>
...
const mock = new Array(10).fill({id: 0,name: "Caleb Sauer", rank:1, location:123, units_sold: 10, revenue_genrated: "$1,500"})
const data = [100];
const options = {
    chart: {
        type: "donut"
...
const statsFilter = [{label:"Option 1", value:"option1"},{label:"Option 2", value:"option2"}]
