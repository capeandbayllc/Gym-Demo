<template>
  <div class="task-page">
    <div class="calendar-item">
      ...
      <div class="w-2/6">
        <month-switcher
          ...
          :startMonth="startOfTheWeek"
        />
      </div>
    </div>

    <div
      class="bg-gradient-to-b from-[#0075C94D] to-black p-10 rounded-2xl border-blue-600 border-2"
    >
      <task-date-switcher
    ...
    <div class="flex my-10 gap-4 w-[60rem]">
      ...
      <div
        :class="all_tab ? 'tab-text' : 'tab-text-selected'"
        @click="selectAll('all')"
      >
        All
      </div>
      <div
        :class="location_tab ? 'tab-text' : 'tab-text-selected'"
        @click="selectAll('location')"
      >
        Location
      </div>
      <div
        :class="employees_tab ? 'tab-text' : 'tab-text-selected'"
        @click="selectAll('employees')"
      >
        Employees
      </div>
    </div>
...
const selectedDateFormatted = computed(() => transformDate(selectedDate.value));
...
const selectAll = (text) => {
  ...
  if (text == "location") {
    location_tab.value
      ? (location_tab.value = false)
      : (location_tab.value = true);
  }
  if (text == "employees") {
    employees_tab.value
      ? (employees_tab.value = false)
      : (employees_tab.value = true);
  }
};
</script>
