<template>
 <tr class="task-table-row-today">
    <td>
      <div class="flex gap-2 border-r-2 border-[#0075C9]">
        <img :src="data.profile_photo_path" class="profile_photo" alt=""> {{ data.first_name }} {{ data.last_name }}
      </div>
    </td>
    <td> <div class="gap-2 border-r-2 border-[#0075C9] pr-3"> Welcome Call</div></td>
    <td><div class="gap-2 border-r-2 border-[#0075C9]">Location {{ Math.floor(Math.random() * (5 - 1 + 1) + 1)}}</div></td>
    <td><div class="gap-2 border-r-2 border-[#0075C9]">18/11/2020</div></td>
    <td>
      <!-- Status -->
      <div class="flex gap-2 border-r-2 border-[#0075C9]"> 
        <div class="dropdown dropdown-end">
            ...
            <div class="dropdown-content top-0 left-2 p-1 menu bg-white w-32 items-start text-[#5A5A5A] text-sm whitespace-nowrap cursor-pointer gap-2 rounded-md">
              ...
              <button  class="flex ml-1"   @click.passive="toggleStatus('Completed')"  >
