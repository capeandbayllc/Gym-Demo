<template>
  <tr class="task-table-row-today">
    ...
    <td><div class="gap-2 border-r-2">Location {{ Math.floor(Math.random() * (5 - 1 + 1) + 1)}}</div></td>
    ...
    <td>
      <div class="flex gap-2 border-r-2"> 
        <div class="dropdown">
            ...
            <div v-else-if="status=='No-Show'" class="text-[#5A5A5A] ml-2 hover:text-[#0075C9] cursor-pointer flex gap-2 " tabindex="0" @click.prevent.stop>
              {{status}} <div class="rounded-full w-6 h-6 bg-[#A8A8A8] m-auto ml-2"></div>
            </div>
            <div v-else-if="status=='Rescheduled'" class="text-[#5A5A5A] ml-2 hover:text-[#0075C9] cursor-pointer flex gap-2 " tabindex="0" @click.prevent.stop>
              {{status}}<div class="rounded-full w-6 h-6 bg-[#00A7FF] m-auto ml-2"></div>
            </div> 
            <div v-else-if="status=='Canceled'" class="text-[#5A5A5A] ml-2 hover:text-[#0075C9] cursor-pointer flex gap-2 " tabindex="0" @click.prevent.stop>
              {{status}}<div class="rounded-full w-6 h-6 bg-[#FF0000] m-auto ml-2"></div>
            </div> 
            <div v-else-if="status=='Overdue'" class="text-[#5A5A5A] ml-2 hover:text-[#0075C9] cursor-pointer flex gap-2 " tabindex="0" @click.prevent.stop>
    ...
    <td>
      <div class="bg-[#0075C9] cursor-pointer border-transparent border text-white text-xs p-1 hover:bg-transparent hover:text-[#0075C9] hover:border hover:border-[#0075C9]">Edit</div>
    </td>
  </tr>
</template>
<style scoped>
