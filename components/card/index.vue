<template>
  ...
  >
    <div
      ...
      v-if="!titleComponent && showTitleContainer"
    ...
    >
      ...
      <div class="card-actions">
        <favorite-btn v-if="options.favorite" v-model="isFavorite" />
        <collapse-btn v-if="options.collapse" v-model="isCollapsed" />
        <add-btn v-if="options.add" v-model="isCollapsed" />
      </div>
    </div>
    ...
    <div
      ...
      :class="bgBlack ? 'bg-black rounded-[16px]' : null"
      v-else
    >
      <slot name="content" />
    </div>
